...
int main()
{
    ...
    for (int i = 0; i < n / 2; ++i)
        t.push_back({1, 0}); // inside
    for (int i = 0; i < n / 2; ++i)
        t.push_back({0, 1}); // outside
    ...
    for (int i = 0; i < loop; ++i)
    {
        ...
        for (int k = 0; k < depth; ++k)
        {
            ...
            if (k == 0)
            {
                nn[k].a = matrix_add(matrix_multi(x0, nn[k].w), nn[k].b);
            }
            else
            {
                nn[k].a = matrix_add(matrix_multi(nn[k - 1].x, nn[k].w), nn[k].b);
            }
            if (k < depth - 1)
            {
        ...
        for (int k = depth - 1; k >= 0; --k)
        {
            ...
            {
                // 3行
                vvd r_hk_ak = calc_r_ReLU(nn[k].a);
                // nn[k].delta = matrix_multi(nn[k + 1].delta, matrix_adm_multi(matrix_t(nn[k + 1].w), r_hk_ak));
                // nn[k].delta = matrix_adm_multi(r_hk_ak, matrix_multi(nn[k + 1].delta, matrix_t(nn[k + 1].w)));
                nn[k].delta = matrix_adm_multi(matrix_multi(nn[k+1].delta, matrix_t(nn[k+1].w)), r_hk_ak);
            ...
            if (k > 0)
            {
                nn[k].rw = matrix_multi(matrix_t(nn[k - 1].x), nn[k].delta);
            }
            else
            {
                nn[k].rw = matrix_multi(matrix_t(x0), nn[k].delta);
            }
        }
    ...
    {
        if (k == 0)
            nn[k].a = matrix_add(matrix_multi(x, nn[k].w), nn[k].b);
        else
            nn[k].a = matrix_add(matrix_multi(nn[k - 1].x, nn[k].w), nn[k].b);
        if (k < depth - 1)
            nn[k].x = hm_ReLU(nn[k].a);
...
void drawing_by_python(vector<layer_t> &nn, int depth)
{
    ...
    for (double x = -6; x <= 6; x += 1)
    {
        for (double y = -6; y <= 6; y += 1)
        {
            ...
            {
                if (k == 0)
                    nn[k].a = matrix_add(matrix_multi(tmp, nn[k].w), nn[k].b);
                else
                    nn[k].a = matrix_add(matrix_multi(nn[k - 1].x, nn[k].w), nn[k].b);
                // if (k < depth-1) nn[k].x = hm_ReLU(nn[k].a);
                if (k < depth - 1)
                    nn[k].x = hm_tanh(nn[k].a);
                else
                    nn[k].x = hm_softmax(nn[k].a);
            }
...
vvd hm_softmax(vvd &x)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        double c = -*max_element(x[i].begin(), x[i].end());
        for (int j = 0; j < m; ++j)
        {
            denominator += exp(x[i][j] + c);
        }
        for (int j = 0; j < m; ++j)
        {
            z = exp(x[i][j] + c) / denominator;
            tmp[i][j] = z;
        }
    }
    return tmp;
    ...
    for (int p = 0; p < n; ++p)
    {
        for (int q = 0; q < m; ++q)
        {
            ...
            for (int j = 0; j < m; j++)
            {
                ...
                tmp_value += (t[p][j] / x[p][j]) * tmp;
            }
            tmp_result[p][q] = tmp_value * (-1) / n;
            // tmp_result[p][q] = tmp_value / (double)(-n);
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < m; j++)
        {
            for (int k = 0; k < m; k++)
            {
                if (j == k)
                {
                    tmp[i][j][k] = x[i][j] * (1 - x[i][k]);
                }
                else
                {
                    tmp[i][j][k] = x[i][j] * (0 - x[i][k]);
                }
            }
        }
    }
...
vvd calc_r_bias(vvd &delta)
{
    ...
    vvd delta_sum(1, vd(m, 0));
    for (int i = 0; i < m; i++)
    {
        for (int j = 0; j < n; j++)
        {
            delta_sum[0][i] += delta[j][i];
        }
    }
    return expansion_bias(delta_sum, n);
}
